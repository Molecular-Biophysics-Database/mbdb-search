...
import json
import os
from convert_yaml_to_json import read_from_file, write_to_file, resolve_ref, create_field_item, build_json_output
...
SAMPLE_YAML_FILE = 'simplified_model.yaml'
...
def test_nested_properties():
    yaml_data = read_from_file('simplified_model.yaml')
    json_output = []
    build_json_output(yaml_data['record'], '', yaml_data.get('$defs', {}), json_output)
    assert any(item['field_path'] == "metadata.general_parameters.record_information.title" for item in json_output)
